...
from src.chart_of_accounts import CHART, CODE_TO_ACCOUNT, NAME_TO_CODE
...
from src import supabase_adapter
from src.llm_classifier import classify_with_openai

st.set_page_config(page_title="📚 AI Bookkeeping v1.2", layout="wide")
...
st.title("📚 AI SaaS Bookkeeping (v1.2)")
st.caption("Upload → AI classify (OpenAI or heuristic) → Human approve → Post → TB → Export")

st.sidebar.header("Settings")
persist_sqlite = st.sidebar.toggle("📦 Save journals to SQLite (bookkeeping.db)", value=False)
if persist_sqlite:
    st.sidebar.success("Persistence: ON (SQLite)")
else:
    st.sidebar.info("Persistence: OFF (in-memory)")

st.sidebar.subheader("AI Settings")
use_openai = st.sidebar.toggle("🤖 Use OpenAI classifier (fallback to heuristics)", value=False)
openai_threshold = st.sidebar.slider("Auto-post confidence threshold (OpenAI)", 0.0, 1.0, 0.90, 0.05)

st.sidebar.subheader("Cloud Storage")
use_supabase = st.sidebar.toggle("☁️ Save to Supabase (journals & suggestions)", value=False)
    ...
    if f is not None:
        try:
            if src_type == "Bank CSV" and f.name.endswith(".csv"):
                df = ocr.load_bank_csv(f.read()); df["source"]="bank_csv"
            elif src_type == "Bank Excel" and (f.name.endswith(".xlsx") or f.name.endswith(".xls")):
                df = ocr.load_bank_excel(f.read()); df["source"]="bank_excel"
            elif src_type == "Bank PDF (digital)" and f.name.endswith(".pdf"):
                df = ocr.load_bank_pdf(f.read()); df["source"]="bank_pdf"
            elif src_type == "Invoice CSV" and f.name.endswith(".csv"):
                df = pd.read_csv(f)
                for col in ["date","description","amount"]:
                    if col not in df.columns: st.error(f"Invoice CSV must include {col}"); st.stop()
                df["date"]=pd.to_datetime(df["date"]).dt.date; df["reference"]=df.get("reference",""); df["source"]="invoice_csv"
            else:
                st.error("File type does not match selected source."); st.stop()
    ...
    if not st.session_state.transactions.empty:
        ...
        if st.button("Run AI on all un-suggested rows"):
            ...
            for i, row in to_classify.iterrows():
                try:
                    if use_openai:
                        acc_name, vat_code, conf, reason = classify_with_openai(str(row["description"]), float(row["amount"]))
                        code = NAME_TO_CODE.get(acc_name.lower(), NAME_TO_CODE.get("sales revenue"))
                        suggestions.append({"date": row["date"],"description": row["description"],"amount": row["amount"],"suggested_account": code,"vat_code": vat_code,"confidence": conf,"reason": reason + " (OpenAI)","link_ref": row.get("reference","")})
                    else:
                    ...
                    suggestions.append({"date": row["date"],"description": row["description"],"amount": row["amount"],"suggested_account": acc_code,"vat_code": vat_code,"confidence": conf,"reason": reason + " (Heuristic)","link_ref": row.get("reference","")})
            ...
            if use_supabase:
                try:
                    supabase_adapter.save_suggestions(sugg_df.to_dict(orient="records"))
                    st.toast("Suggestions saved to Supabase", icon="☁️")
                except Exception as e:
...
with tab_review:
    ...
    if st.session_state.suggestions.empty:
        ...
        edited = st.data_editor(
            ...
            column_config={
                "account_label": st.column_config.SelectboxColumn("Account (edit as needed)", options=list(code_to_label.values()), required=True),
                "vat_code": st.column_config.SelectboxColumn("VAT Code", options=["STD","ZERO","EXEMPT","NONE"], required=True),
        ...
        if st.button("Post Selected"):
            ...
            for idx in post_idx:
                ...
                amt = float(row["amount"]); memo = str(row["description"]); vat_code = str(row["vat_code"]).upper()
                conf = float(row["confidence"] or 1.0); reason = str(row.get("reason","") or "")
                if counter_acc == 1000:
                    if amt < 0:
            ...
            if persist_sqlite:
                jdf_now = st.session_state.ledger.to_dataframe()
                db_sqlite.save_journal_rows(jdf_now.tail(count*2).to_dict(orient="records"))
                st.toast("Saved to SQLite", icon="✅")
            if use_supabase:
                try:
                    supabase_adapter.save_journals(jdf_now.tail(count*2).to_dict(orient="records"))
                    st.toast("Journals saved to Supabase", icon="☁️")
                except Exception as e:
                    st.warning(f"Supabase save failed: {e}")
