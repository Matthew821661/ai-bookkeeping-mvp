...
def classify(description: str, amount: float) -> Tuple[int, str, float, str]:
    ...
    if "interest" in desc and amount > 0:
        return NAME_TO_CODE["interest received"], "EXEMPT", 0.95, "Matched 'interest' inflow: VAT exempt"
    for pattern, account_name in KEYWORD_TO_ACCOUNT.items():
        if re.search(pattern, desc):
            ...
            if amount > 0 and account_name == "Sales Revenue":
                vat = "STD"
            return NAME_TO_CODE[account_name.lower()], vat, 0.75, f"Heuristic match → {account_name}"
    if amount > 0:
        return NAME_TO_CODE["sales revenue"], "STD", 0.55, "Fallback: positive → revenue"
    else:
        return NAME_TO_CODE["cost of sales"], "STD", 0.55, "Fallback: negative → expense"
